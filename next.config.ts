...
const nextConfig: NextConfig = {
  ...
  async headers() {
    return [
      ...
      // robots.txt is generated by Next's MetadataRoute.Robots which
      // also has no Response handle. Defaults to no-cache; override to
      // a 1-day cache so crawlers don't re-fetch on every visit. The
      // file is small but it's hit by every crawler on every crawl
      // cycle, and changes only when admin-disallow paths shift —
      // a 1-day TTL with a week of SWR fallback is the right trade.
      {
        source: "/robots.txt",
        headers: [
          {
