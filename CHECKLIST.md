...
- [ ] Replace the typeset "F" monogram with a designer-supplied SVG when the final logo lands. Drop the file into `public/icon.svg` (512×512 viewBox) and re-run `node scripts/generate-icons.mjs` — the script will rasterise it into every PNG variant. The current monogram already uses the confirmed brand palette (`siteConfig.brand.foreground` + `…background`), so dropping in a real mark is the only step left.
...
- [ ] All icon URLs return 200: `/icon.svg`, `/favicon.ico`, `/favicon-16x16.png`, `/favicon-32x32.png`, `/apple-touch-icon.png`, `/icon-192.png`, `/icon-512.png`, `/icon-maskable-192.png`, `/icon-maskable-512.png`.
- [ ] Android Chrome → Add to Home Screen: installed icon, app name, and splash screen render with the brand monogram + cream background. Launching the installed app lands on `/ka`.
- [ ] iOS Safari → Share → Add to Home Screen: the home-screen tile shows the apple-touch-icon (no transparent corners) and the title under the icon is "Furnituremodern", not the per-page `<title>`.
...
- [ ] **Every active product has had its stock placeholder swapped for real photography** — follow the `Replacing stock photos with real product photography` workflow in README.md section 8 for every row where `product_images.source IS NOT NULL`. Re-query Supabase Studio: `SELECT count(*) FROM product_images WHERE source IS NOT NULL AND product_id IN (SELECT id FROM products WHERE is_published = true);` should return 0 before launch.
- [ ] **Zero references to `scripts/stock-photos-*` in production data** — `grep -r 'stock-photos-source\|stock-photos-prepared' supabase/ content/ lib/` should return only the documented script files under `scripts/`. The DB itself stores `stock/<filename>.jpg` keys but those resolve to real Storage objects, not the local prep directories.
- [ ] **Re-run Lighthouse mobile after the swap** on `/ka`, `/ka/sofas`, one real `/ka/sofas/[product]`, `/ka/privacy`, `/en` — Performance ≥ 95 on every URL (record below; the `Lighthouse mobile` line above carries the four-score record). Run the desktop pass too as a sanity check; aim ≥ 98 Performance there but no hard gate.
...
- ~~**Global header + footer redesign**~~ — Done (Phase 5 Task 5.7). Sticky header with scroll-aware shrink (transparent at top → frosted on scroll), brand monogram + name, primary nav with accent-underline active state, language switcher (KA / EN inline), mobile drawer with focus trap + ESC + backdrop close. Footer rebuilt as a 5-column editorial grid (Brand, Explore, Customer, Visit, Connect) with a bottom band carrying © and a mirrored LanguageSwitcher. Files: `components/layout/header.tsx`, `components/layout/footer.tsx`, `components/layout/LanguageSwitcher.tsx`, `components/layout/BrandMark.tsx`, `components/layout/HeaderScrollEffect.tsx`, `components/layout/NavLink.tsx`, `components/layout/mobile-nav.tsx`, `components/layout/desktop-nav.tsx`, `components/layout/social-icons.tsx`. Scroll listener is rAF-throttled and toggles `data-scrolled` on a single element — no React re-render on scroll.
...
## Pre-launch confirmations — Phase 5 Task 5.7 (chrome redesign)

The header (`components/layout/header.tsx`) now ships sticky-with-shrink and overlays the home Hero. The footer (`components/layout/footer.tsx`) renders a five-column desktop grid with brand mark, Explore, Customer, Visit (locale-aware contact), and Connect (social). A shared `LanguageSwitcher` (`components/layout/LanguageSwitcher.tsx`) sits in both the header and the footer bottom band. Confirm before launch:

- [ ] **Hero overlay**: open `/ka` and `/en`. Sticky header should be transparent at scroll-y=0, then transition to a frosted, tighter chrome with a subtle shadow once you scroll past ~80px. Border-bottom appears in the scrolled state. Smooth 200ms transitions; reduced-motion preference collapses transitions to instant via the global rule in `app/globals.css`.
- [ ] **Active state**: navigating to `/ka/sofas` (or any category) marks the matching nav link with the accent underline (`data-active="true"`, `aria-current="page"`). Walking back to `/ka` clears the underline on category links and applies it to Home only.
- [ ] **Mobile drawer**: at viewport ≤ 768px, the hamburger opens a right-side drawer. Confirm: focus moves to the close button on open, ESC closes, backdrop click closes, link click closes, Tab cycles only inside the drawer (focus trap), focus is restored to the trigger on close. The drawer carries the address, opening hours, phone, email and a `drawer`-variant LanguageSwitcher.
- [ ] **Language switcher path preservation**: open `/ka/sofas/<slug>`, click the EN link — should land on `/en/sofas/<slug>` (same path under the alternate locale). Repeat from `/ka/privacy`, `/ka/search`, the home page, and a 404 path. Same component renders in the header (desktop), the footer bottom band, and the mobile drawer.
- [ ] **Footer content from siteConfig**: every column reads its content from `lib/site-config.ts` (address, phone, email, opening hours, social URLs). Update `siteConfig.contact.*` and the footer reflects the change without code edits.
- [ ] **No bundle regression**: home-page first-load JS does not regress. The drawer animates with CSS-only transforms (no framer-motion). The scroll-state controller (`HeaderScrollEffect`) is a tiny client island that toggles a single `data-scrolled` attribute via `requestAnimationFrame` — never re-renders React on scroll.
- [ ] **Skip link is the first focusable element**: tab once on a fresh page — the visible "Skip to main content" link should appear before the brand mark.
